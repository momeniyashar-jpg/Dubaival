...
// bucket: optional extra key segment so a handler can apply a second,
// independent limit (e.g. a stricter one for one specific action) without
// sharing counters with its own top-level per-IP check.
function isRateLimited(req, windowMs, max, bucket) {
  var now = Date.now();
  ...
  var ip = getIp(req);
  var key = ip + (bucket ? ":" + bucket : "") + ":" + Math.floor(now / windowMs);
  if (!_store[key]) {
...
function rateLimitExceeded(req, res, windowMs, max, bucket) {
  if (isRateLimited(req, windowMs, max, bucket)) {
