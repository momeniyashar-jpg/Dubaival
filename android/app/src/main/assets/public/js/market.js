...
function getListingPitchPrompt(propDesc,val,area){
  ...
  var user=propDesc+". EXACT NUMBERS FROM OUR DLD-VERIFIED ENGINE (use these, do NOT invent your own): Fair value: AED "+val.fairPrice.toLocaleString()+". Recommended listing price (fair value + a competitive premium): AED "+recommendedList.toLocaleString()+". Realistic floor if it needs repricing later: AED "+floorList.toLocaleString()+". Market PSF: AED "+val.adjPSF.toLocaleString()+". Confidence: "+val.confScore+"% ("+val.confTier.label+"). Gross yield at fair value: "+val.grossYield+"%. "+areaCtx+"MARKET PACE: "+(domFast?"a FAST-moving area — emphasize genuine buyer competition and how correct day-one pricing captures the strongest early interest":domSlow?"a SLOWER-moving area — emphasize a strong marketing/pricing plan as the real differentiator, not manufactured urgency":"a MODERATE-pace area")+".";
...
function renderMarket(){
  const cl=C();
  const wrap=div({padding:"0",maxWidth:"960px",margin:"0 auto",width:"100%",boxSizing:"border-box"});

  // Inject premium CSS once
  if(!document.getElementById('dv-mkt-css')){
    var _st=document.createElement('style');_st.id='dv-mkt-css';
    _st.textContent=[
      '.dv-mfu{animation:dvFadeUp 0.55s cubic-bezier(0.16,1,0.3,1) both}',
      '.dv-mfu-1{animation-delay:0.04s}.dv-mfu-2{animation-delay:0.10s}.dv-mfu-3{animation-delay:0.17s}',
      '.dv-mfu-4{animation-delay:0.24s}.dv-mfu-5{animation-delay:0.32s}.dv-mfu-6{animation-delay:0.40s}',
      '.dv-sc:hover{background:rgba(255,255,255,0.055)!important}',
      '.dv-rc:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,0.3)!important}'
    ].join('');
  ...
  (function(){
    ...
    var dSec=el('div',{className:'dv-mfu dv-mfu-1',style:{padding:'16px 16px 0',marginBottom:'0'}});

    // Premium header
    var hdr=el('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'16px'}});
    var hdrL=el('div',{});
    hdrL.appendChild(div({fontSize:'10px',color:'#6B7A9E',fontWeight:'700',fontFamily:"'Inter',sans-serif",letterSpacing:'0.10em',textTransform:'uppercase',marginBottom:'4px'},'Market Intelligence'));
    hdrL.appendChild(div({fontSize:'22px',fontWeight:'800',color:'#FFFFFF',fontFamily:"'Space Grotesk',sans-serif",letterSpacing:'-0.02em',lineHeight:'1'},'Dubai Live Dashboard'));
    hdr.appendChild(hdrL);
    var liveTag=el('div',{style:{display:'flex',alignItems:'center',gap:'5px',background:'rgba(0,200,150,0.08)',border:'1px solid rgba(0,200,150,0.20)',borderRadius:'20px',padding:'5px 11px',flexShrink:'0'}});
    var liveDot=el('div',{style:{width:'6px',height:'6px',borderRadius:'50%',background:'#00C896',animation:'dvPulse 2s ease infinite'}});
    liveTag.appendChild(liveDot);
    liveTag.appendChild(span({fontSize:'10px',color:'#00C896',fontFamily:"'Space Grotesk',sans-serif",fontWeight:'700',letterSpacing:'0.08em'},'LIVE'));
    hdr.appendChild(liveTag);
    dSec.appendChild(hdr);

    // Row 1: 5 stat cards with count-up — premium left-border style
    var statColors=['#D4A843','#3B82F6','#00C896','#8B5CF6','#F59E0B'];
    var r1=el('div',{style:{display:'grid',gridTemplateColumns:window.innerWidth<500?'repeat(auto-fit,minmax(100px,1fr))':'1fr 1fr 1fr 1fr 1fr',gap:'8px',marginBottom:'16px'}});
    [{l:'Buildings',v:bCnt,fmt:function(v){return v.toLocaleString();}},{l:'Areas',v:aCnt,fmt:String},{l:'Avg PSF',v:avgPsf,fmt:function(v){return 'AED '+v.toLocaleString();}},{l:'Avg Yield',v:parseFloat(avgYield),fmt:function(v){return v.toFixed(1)+'%';}},{l:'Growth 1Y',v:parseFloat(avgGr),fmt:function(v){return v.toFixed(1)+'%';}}].forEach(function(s,idx){
      var c=statColors[idx];
      var card=el('div',{className:'dv-sc',style:{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderLeft:'3px solid '+c,borderRadius:'12px',padding:'14px 10px',textAlign:'center',transition:'background 0.2s ease,border-color 0.2s ease'}});
      var numEl=el('div',{style:{color:c,fontSize:'20px',fontWeight:'800',fontFamily:"'JetBrains Mono',monospace",fontFeatureSettings:"'tnum'",lineHeight:'1',marginBottom:'5px'}});
      numEl.textContent='—';
      card.appendChild(numEl);
      card.appendChild(div({color:'#9BA8C8',fontSize:'9px',letterSpacing:'0.07em',textTransform:'uppercase',fontFamily:"'Inter',sans-serif",fontWeight:'600'},s.l));
      r1.appendChild(card);
    ...
    distHdr.appendChild(span({color:cl.gold,fontSize:'11px',letterSpacing:'0.1em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace",fontWeight:'700'},'Price & Yield Distribution'));
    ...
    gHdr.appendChild(span({color:'#22C55E',fontSize:'11px',letterSpacing:'0.1em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace",fontWeight:'700'},'Growth Heatmap'));
    ...
    var gTopCard=el('div',{style:{background:'rgba(34,197,94,0.04)',border:'1px solid rgba(34,197,94,0.15)',borderRadius:'10px',padding:'10px',overflow:'hidden'}});
    gTopCard.appendChild(div({color:'#22C55E',fontSize:'10px',fontWeight:'700',letterSpacing:'0.06em',fontFamily:"'Space Grotesk',monospace",marginBottom:'8px'},'Top 10 Growth'));
    gArr.slice(0,10).forEach(function(g,i){
      ...
      row.appendChild(span({color:cl.sub,fontSize:'9px',fontFamily:"'Space Grotesk',monospace",width:'14px',textAlign:'right',flexShrink:'0',display:'inline-block'},String(i+1)));
      row.appendChild(span({color:cl.subHi,fontSize:'10px',fontFamily:"'Inter',sans-serif",flex:'1',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',display:'inline-block',minWidth:'0'},g.name.length>12?g.name.substring(0,12)+'…':g.name));
      row.appendChild(span({color:'#22C55E',fontSize:'10px',fontWeight:'700',fontFamily:"'Space Grotesk',monospace",flexShrink:'0',display:'inline-block',textAlign:'right'},'+'+(g.g).toFixed(1)+'%'));
      gTopCard.appendChild(row);
    });
    gWrap.appendChild(gTopCard);

    var gBotCard=el('div',{style:{background:'rgba(239,68,68,0.04)',border:'1px solid rgba(239,68,68,0.15)',borderRadius:'10px',padding:'10px',overflow:'hidden'}});
    gBotCard.appendChild(div({color:'#EF4444',fontSize:'10px',fontWeight:'700',letterSpacing:'0.06em',fontFamily:"'Space Grotesk',monospace",marginBottom:'8px'},'Bottom 10 Growth'));
    gArr.slice(-10).reverse().forEach(function(g,i){
      var pct=Math.min(100,Math.abs(g.g)*3);
      var row=el('div',{style:{display:'flex',alignItems:'center',gap:'4px',marginBottom:'3px'}});
      row.appendChild(span({color:cl.sub,fontSize:'9px',fontFamily:"'Space Grotesk',monospace",width:'14px',textAlign:'right',flexShrink:'0',display:'inline-block'},String(i+1)));
      row.appendChild(span({color:cl.subHi,fontSize:'10px',fontFamily:"'Inter',sans-serif",flex:'1',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',display:'inline-block',minWidth:'0'},g.name.length>12?g.name.substring(0,12)+'…':g.name));
      row.appendChild(span({color:'#EF4444',fontSize:'10px',fontWeight:'700',fontFamily:"'Space Grotesk',monospace",flexShrink:'0',display:'inline-block',textAlign:'right'},(g.g>=0?'+':'')+g.g.toFixed(1)+'%'));
    ...
    [{title:'Hottest Areas',data:hottest,vFn:function(d){return(d.y+d.g).toFixed(1);},color:'#F97316'},
     ...
     {title:'Highest Growth',data:highGr,vFn:function(d){return'+'+d.g.toFixed(1)+'%';},color:'#A78BFA'}].forEach(function(cat){
      var card=el('div',{style:{background:cl.surface,border:'1px solid '+cl.border,borderRadius:'10px',padding:'10px',overflow:'hidden'}});
      card.appendChild(div({color:cat.color,fontSize:'10px',fontWeight:'700',letterSpacing:'0.06em',fontFamily:"'Space Grotesk',monospace",marginBottom:'6px'},cat.title));
      cat.data.forEach(function(d,i){
        var row=el('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'2px 0',gap:'4px'}});
        row.appendChild(span({color:cl.subHi,fontSize:'10px',fontFamily:"'Inter',sans-serif",overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:'1',minWidth:'0'},''+(i+1)+'. '+(d.name.length>11?d.name.substring(0,11)+'…':d.name)));
        row.appendChild(span({color:cat.color,fontSize:'9px',fontWeight:'700',fontFamily:"'Space Grotesk',monospace",flexShrink:'0',whiteSpace:'nowrap'},cat.vFn(d)));
    ...
    rentalHdr.appendChild(span({color:'#8B5CF6',fontSize:'11px',letterSpacing:'0.1em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace",fontWeight:'700'},'Rental Market Snapshot'));
    ...
    var r4=el('div',{style:{display:'grid',gridTemplateColumns:window.innerWidth<500?'1fr 1fr':'1fr 1fr 1fr 1fr',gap:'8px',marginBottom:'12px'}});
    [{l:'Areas w/ Rental Data',v:String(cntR),c:'#8B5CF6'},{l:'Avg 1BR Rent',v:'AED '+avgR1.toLocaleString(),c:'#8B5CF6'},{l:'Avg 2BR Rent',v:'AED '+avgR2.toLocaleString(),c:'#8B5CF6'},{l:'Avg Monthly 1BR',v:'AED '+Math.round(avgR1/12).toLocaleString(),c:'#8B5CF6'}].forEach(function(s){
      ...
      card.appendChild(div({color:cl.sub,fontSize:'10px',letterSpacing:'0.08em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace",marginBottom:'4px'},s.l));
    ...
    comHdr.appendChild(span({color:'#3B82F6',fontSize:'11px',letterSpacing:'0.1em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace",fontWeight:'700'},'Commercial & Land Market'));
    ...
    var r5=el('div',{style:{display:'grid',gridTemplateColumns:window.innerWidth<500?'1fr 1fr':'1fr 1fr 1fr 1fr',gap:'8px',marginBottom:'12px'}});
    ...
    aiHdr.appendChild(span({color:'#F59E0B',fontSize:'11px',letterSpacing:'0.1em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace",fontWeight:'700'},'AI Market Intelligence · Groq'));
    ...
    (function(){
      ...
      var r6=el('div',{style:{display:'grid',gridTemplateColumns:window.innerWidth<500?'1fr 1fr':'1fr 1fr 1fr 1fr',gap:'8px',marginBottom:'12px'}});
      ...
      if(momKeys.length>0){
        ...
        var moverGrid=el('div',{style:{display:'grid',gridTemplateColumns:window.innerWidth<500?'1fr 1fr':'1fr 1fr 1fr',gap:'6px',marginBottom:'12px'}});
  ...
  const isAllView=window.CHART_STATE.view==="All";

  // Header
  chartWrap.appendChild(div({display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"},[
    span({color:cl.gold,fontSize:"10px",letterSpacing:"0.14em",textTransform:"uppercase",fontFamily:"'Space Grotesk',monospace"},
      isAllView?"Market Cycle Index · "+MARKET_CYCLE_INDEX.baseYear+"–Present":"PSF Trend · 3-Year History · DLD Data"),
    span({color:cl.sub,fontSize:"8.5px",fontFamily:"'Space Grotesk',monospace"},
      isAllView?"Illustrative · base 100 = "+MARKET_CYCLE_INDEX.baseYear:"Source: DLD 2022–2026"),
  ]));

  // Area selector — hidden in "All" view since the long-run index is a
  // citywide reconstruction, not derived per-area (see MARKET_CYCLE_INDEX
  ...
  if(!isAllView){
    ...
    selRow.appendChild(span({color:cl.sub,fontSize:"10.5px",fontFamily:"'Inter',sans-serif",display:"flex",alignItems:"center"},"Dubai — citywide residential index"));
  }

  // View buttons
  [{k:"6M",l:"6M"},{k:"1Y",l:"1Y"},{k:"3Y",l:"3Y"},{k:"All",l:"All"}].forEach(function(v){
  ...
  let labels,values,unitLabel,changeLabel;
  if(isAllView){
    labels=MARKET_CYCLE_INDEX.years.map(String);
    values=MARKET_CYCLE_INDEX.index.slice();
    unitLabel="Index";
    changeLabel=(MARKET_CYCLE_INDEX.years[MARKET_CYCLE_INDEX.years.length-1]-MARKET_CYCLE_INDEX.baseYear)+"Y Change";
  }else{
    ...
    labels=cData.labels.slice(-nMonths);
    values=cData.data.slice(-nMonths);
    unitLabel="PSF";
    changeLabel=nMonths+"M Change";
  }
  ...
  [
    {l:isAllView?"Current Index":"Current PSF",v:(isAllView?"":"AED ")+lastPSF.toLocaleString(),c:cl.gold},
    {l:changeLabel,v:(isUp?"+":"")+changePct+"%",c:isUp?cl.green:cl.red},
  ...
  const yRound=isAllView?10:100;
  const yMin=Math.floor(minPSF*0.97/yRound)*yRound;
  const yMax=Math.ceil(maxPSF*1.03/yRound)*yRound;
  ...
  // Y grid lines
  const svgGridStroke=hexAlpha(cl.sub,0.2);
  const svgTextFill=hexAlpha(cl.sub,0.6);
  const yStep=Math.round((yMax-yMin)/4/yRound)*yRound||yRound*5;
  const gridLines=[];
  for(let y=yMin;y<=yMax;y+=yStep){
    const py=yPos(y);
    gridLines.push('<line x1="'+padL+'" y1="'+py+'" x2="'+(chartW-padR)+'" y2="'+py+'" stroke="'+svgGridStroke+'" stroke-width="1"/>');
    gridLines.push('<text x="'+(padL-4)+'" y="'+(py+4)+'" fill="'+svgTextFill+'" font-size="9" text-anchor="end" font-family="monospace">'+y+'</text>');
  }

  // X labels — every 6 months for the monthly views, every ~4 years for "All"
  const xLabels=[];
  const xLabelStep=isAllView?Math.max(1,Math.floor(n/7)):Math.max(1,Math.floor(n/6));
  for(let i=0;i<n;i+=xLabelStep){
    const lbl=isAllView?labels[i]:labels[i].slice(0,7);
    xLabels.push('<text x="'+xPos(i)+'" y="'+(padT+plotH+16)+'" fill="'+svgTextFill+'" font-size="8" text-anchor="middle" font-family="monospace">'+lbl+'</text>');
  }
  ...
  const svgWrap=el("div",{style:{borderRadius:"8px",overflow:"hidden",background:cl.surface,border:"1px solid "+cl.border}});
  svgWrap.innerHTML=svgHTML;
  chartWrap.appendChild(svgWrap);
  
  chartWrap.appendChild(div({marginTop:"8px",color:cl.sub,fontSize:"8px",fontFamily:"'Space Grotesk',monospace",textAlign:"right"},
    isAllView
      ?"Illustrative index reconstructed from Dubai's widely-documented market cycle (freehold boom, 2008 GFC, recovery, correction, COVID dip, post-pandemic super-cycle) — not exact monthly DLD figures before 2023. "+labels[0]+" to "+labels[labels.length-1]
      :"DLD verified transactions · "+labels[0]+" to "+labels[labels.length-1]));
  ...
  // The old static "Market Cycle · 20-Year History" text list (the same 7
  // eras now in MARKET_CYCLE_INDEX.eras above) was superseded 2026-07-15 by
  // the PSF Trend chart's "All" view — showing the same cycle as a real
  // chart, not duplicated as a second text list further down the page.
  wrap.appendChild(div({background:cl.surface,border:"1px solid "+cl.border,borderRadius:"14px",padding:"18px"},[
    span({color:cl.gold,fontSize:"10px",letterSpacing:"0.14em",textTransform:"uppercase",fontFamily:"'Space Grotesk',monospace",display:"block",marginBottom:"10px"},"Current Market Conditions"),
    div({color:cl.text,fontSize:"12px",fontFamily:"'Inter',sans-serif",lineHeight:"1.7"},
      _currentMonthYear()+": Post-geo correction. Buyer leverage open 3–6 months. Geo adjustment "+Math.round(LIVE_GEO.adj*100)+"% applied. See the full 20-year cycle in the PSF Trend chart's \"All\" view above."
    ),
  ]));
...
function renderTrackRecord(){
  ...
  const CASE_STUDIES=[
    ...
    {building:"Princess Tower",label:"Princess Tower",area:"Dubai Marina",unitType:"2BR",size:1300,sold:2170304,date:"2025",source:"https://www.bayut.com/for-sale/2-bedroom-apartments/dubai/dubai-marina/princess-tower/"},
    ...
    {building:"Binghatti House",label:"Binghatti House",area:"Jumeirah Village Circle",unitType:"1BR",size:750,sold:1052209,date:"2025",source:"https://www.bayut.com/for-sale/1-bedroom-apartments/dubai/jumeirah-village-circle-jvc/binghatti-house/"},
    ...
    {building:"",label:"JVC — Area Average",area:"Jumeirah Village Circle",unitType:"Studio",size:450,sold:600000,date:"2025",source:"https://www.bayut.com/for-sale/studio-apartments/dubai/jumeirah-village-circle-jvc/"},
    ...
    {building:"",label:"Dubai Sports City — Area Average",area:"Dubai Sports City",unitType:"1BR",size:750,sold:860775,date:"2025",source:"https://www.bayut.com/for-sale/1-bedroom-apartments/dubai/dubai-sports-city/"},
    {building:"",label:"JLT — Area Average",area:"Jumeirah Lake Towers",unitType:"1BR",size:960,sold:1470000,date:"2025",source:"https://www.bayut.com/for-sale/1-bedroom-apartments/dubai/jumeirah-lake-towers-jlt/"},
    {building:"Tilal Al Furjan (Nakheel)",label:"Tilal Al Furjan",area:"Al Furjan",unitType:"Villa-4BR",size:3934,sold:7560000,date:"2025",source:"https://www.bayut.com/for-sale/4-bedroom-villas/dubai/al-furjan/tilal-al-furjan/"},
    {building:"",label:"Al Furjan — Area Average",area:"Al Furjan",unitType:"2BR",size:1300,sold:1650000,date:"2025",source:"https://www.bayut.com/for-sale/2-bedroom-apartments/dubai/al-furjan/"},
    ...
    {building:"",label:"Dubai Hills Estate — Area Average",area:"Dubai Hills Estate",unitType:"2BR",size:1200,sold:2260770,date:"2025",source:"https://www.bayut.com/for-sale/2-bedroom-apartments/dubai/dubai-hills-estate/"},
  ];
  ...
  wrap.appendChild(div({background:cl.surface,border:"1px solid "+cl.border,borderRadius:"14px",padding:"18px",marginBottom:"14px"},[
    ...
    div({marginTop:"12px",background:cl.goldFaint,border:"1px solid "+cl.goldDim,borderRadius:"8px",padding:"10px 14px",color:cl.text,fontSize:"11.5px",fontFamily:"'Inter',sans-serif",lineHeight:"1.7"},
...
// Method D — Grade-Weighted Hedonic Range engine
function computeAreaPriceRange(area, beds, mode) {
  var SQFT={
    "Studio":{ entry:[380,560],  mid:[560,780],   premium:[780,1300]  },
    "1 BR":  { entry:[580,820],  mid:[820,1100],  premium:[1100,1700] },
    "2 BR":  { entry:[820,1150], mid:[1150,1550], premium:[1550,2600] },
    "3 BR":  { entry:[1300,1700],mid:[1700,2300], premium:[2300,4200] },
  ...
  if(mode==="rent"){
    ...
    return{lo:r5k(base*0.78),hi:r5k(base*1.45),
      ...
      midRange:[r5k(base*0.95),r5k(base*1.18)],
      premiumRange:[r5k(base*1.18),r5k(base*1.45)],
      base:r5k(base),isRent:true,count:0};
  }

  var ePx=[],mPx=[],pPx=[],allPx=[];
  Object.values(DB).forEach(function(b){
    if(b.a!==area)return;
    var tier=GTIER[b.g]||"mid"; var sq=sqDef[tier];
    var lo=(b.lo||b.p*0.83)*sq[0]; var hi=(b.hi||b.p*1.18)*sq[1];
    allPx.push(lo,hi);
    if(tier==="entry")ePx.push(lo,hi);
    if(tier==="mid")mPx.push(lo,hi);
    if(tier==="premium")pPx.push(lo,hi);
  });
  function r50(n){return Math.round(n/50000)*50000;}
  function mn(a){return a.reduce(function(m,v){return v<m?v:m;},a[0]);}
  function mx(a){return a.reduce(function(m,v){return v>m?v:m;},a[0]);}
  if(!allPx.length){
    var sq=sqDef.mid;
    return{lo:r50(aData.psf*sq[0]),hi:r50(aData.psf*sq[1]),entryRange:null,midRange:null,premiumRange:null,count:0};
  }
  allPx.sort(function(a,b){return a-b;});
  return{
    lo:r50(allPx[Math.floor(allPx.length*0.10)]),
    hi:r50(allPx[Math.min(Math.floor(allPx.length*0.90),allPx.length-1)]),
    entryRange:ePx.length?[r50(mn(ePx)),r50(mx(ePx))]:null,
    midRange:mPx.length?[r50(mn(mPx)),r50(mx(mPx))]:null,
    premiumRange:pPx.length?[r50(mn(pPx)),r50(mx(pPx))]:null,
    count:Math.floor(allPx.length/2)
  };
}

// 2026-07-14 redesign, user-requested: Quick Check's job is no longer "is my
// specific price a fair deal" (an abstract area-wide verdict with no building
// name attached) — it's "given my budget, which actual buildings in this area
// can I buy/rent into, ranked best-first." Reuses the exact same real
// building-level estimator (estimateBuildingRentYield, js/valuation.js)
// Smart Discovery already relies on — no new/fabricated numbers, just a new
// budget-first filter+rank layer over real per-building PSF/grade data.
function _qcRecommendBuildings(area,beds,mode,budget){
  ...
  for(var k in DB){
    ...
    candidates.push({name:k,g:bData.g,p:bData.p,sc:bData.sc,amount:amount,fits:amount<=budget});
  }
  if(!candidates.length)return null;
  var affordable=candidates.filter(function(c){return c.fits;});
  affordable.sort(function(a,b){return(gradeRank[b.g]||0)-(gradeRank[a.g]||0)||b.amount-a.amount;});
  var picks=affordable.slice(0,3);
  if(picks.length<3){
    var above=candidates.filter(function(c){return!c.fits;}).sort(function(a,b){return a.amount-b.amount;});
    picks=picks.concat(above.slice(0,3-picks.length));
  }
  return{picks:picks,totalCandidates:candidates.length,anyAffordable:affordable.length>0};
}

// Building recommendation cards — the tool's headline output since the
// 2026-07-14 redesign: real, ranked, NAMED buildings the budget can reach,
// not just an abstract area-wide range. Grade-ranked among affordable picks
// (best grade first) so the top pick is genuinely the best option within
// budget, not just the cheapest.
function _renderQCBuildingPicks(qc, qs, cl){
  var res=qs.buildingPicks; if(!res||!res.picks||!res.picks.length)return;
  var isRent=qs.mode==="rent";
  function fmtP(n){
    if(!n&&n!==0)return"—";
    if(n>=1000000)return"AED "+(n/1000000).toFixed(2).replace(/0$/,"").replace(/\.$/,"")+"M";
    return"AED "+Math.round(n).toLocaleString();
  }
  var wrap=el("div",{style:{marginTop:"16px",marginBottom:"4px"}});
  wrap.appendChild(el("div",{style:{color:cl.gold,fontSize:"10px",fontWeight:"700",fontFamily:"'Space Grotesk',monospace",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"8px"}},
    (isRent?"Buildings you can rent":"Buildings you can buy")+" with AED "+parseInt(qs.price).toLocaleString()));
  res.picks.forEach(function(p,i){
    var card=el("div",{style:{background:cl.surface,border:"1px solid "+(p.fits?cl.goldDim:cl.border),borderRadius:"12px",padding:"12px 14px",marginBottom:"8px",cursor:"pointer"}});
    var top=el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"10px"}});
    var left=el("div",{});
    left.appendChild(el("div",{style:{color:cl.white,fontSize:"13px",fontWeight:"700",fontFamily:"'Inter',sans-serif",textTransform:"capitalize"}},p.name));
    left.appendChild(el("div",{style:{color:cl.sub,fontSize:"10px",fontFamily:"'Space Grotesk',monospace",marginTop:"2px"}},(p.g||"—")+" grade · AED "+Math.round(p.p).toLocaleString()+"/sqft"));
    top.appendChild(left);
    var right=el("div",{style:{textAlign:"right",flexShrink:"0"}});
    right.appendChild(el("div",{style:{color:p.fits?"#10B981":"#F59E0B",fontSize:"14px",fontWeight:"800",fontFamily:"'Space Grotesk',monospace"}},fmtP(p.amount)+(isRent?"/yr":"")));
    right.appendChild(el("div",{style:{color:p.fits?(i===0?"#10B981":cl.muted):"#F59E0B",fontSize:"9px",fontFamily:"'Space Grotesk',monospace",marginTop:"2px"}},
      p.fits?(i===0?"Top pick":"Within budget"):"Above budget"));
  ...
  if(!res.anyAffordable){
    wrap.appendChild(el("div",{style:{color:cl.sub,fontSize:"10.5px",fontFamily:"'Inter',sans-serif",fontStyle:"italic",marginBottom:"6px"}},
      "No tracked building in "+qs.area+" fits this budget for "+(qs.beds||"2 BR")+" — showing the closest options above."));
  }
  wrap.appendChild(el("div",{style:{color:cl.muted,fontSize:"9px",fontFamily:"'Inter',sans-serif"}},
    "Based on market data (building grade & PSF) — not a live listing check. Confirm availability with an agent."));
  qc.appendChild(wrap);
}

// Shared QC result renderer (building picks + area range context)
function _renderQCResult(qc, qs, cl){
  _renderQCBuildingPicks(qc, qs, cl);
  ...
  var rangeCard=el("div",{style:{marginTop:"16px",borderRadius:"14px",overflow:"hidden",border:"1px solid "+cl.goldDim}});
  // Header — main range
  var hdr=el("div",{style:{background:"rgba(201,168,76,0.07)",padding:"16px",textAlign:"center",borderBottom:"1px solid "+cl.goldDim}});
  hdr.appendChild(el("div",{style:{color:cl.sub,fontSize:"9px",fontFamily:"'Space Grotesk',monospace",letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:"6px"}},
    (qs.area||"Area")+" · "+(qs.beds||"2 BR")+(isRent?" · Annual Rent range":"")));
  var mainRng=el("div",{style:{color:cl.gold,fontSize:"22px",fontWeight:"900",fontFamily:"'Space Grotesk',monospace",letterSpacing:"-0.02em"}});
  mainRng.textContent=fmtP(rng.lo)+" – "+fmtP(rng.hi);
  hdr.appendChild(mainRng);
  if(isRent){
    hdr.appendChild(el("div",{style:{color:cl.sub,fontSize:"11px",fontFamily:"'Inter',sans-serif",marginTop:"4px"}},
      "Monthly: AED "+Math.round(rng.lo/12).toLocaleString()+" – AED "+Math.round(rng.hi/12).toLocaleString()));
  }
  if(rng.count>0){
    hdr.appendChild(el("div",{style:{color:cl.muted,fontSize:"9px",fontFamily:"'Space Grotesk',monospace",marginTop:"6px"}},rng.count+" buildings analyzed"));
  }
  rangeCard.appendChild(hdr);
  // Tier breakdown
  [{key:"entryRange",icon:"🏢",label:"Entry",sub:"C – B grade",color:"#60A5FA"},
   {key:"midRange",icon:"⭐",label:"Mid",sub:"B+ – A- grade",color:cl.gold},
   {key:"premiumRange",icon:"👑",label:"Premium",sub:"A – Ultra grade",color:"#A78BFA"}
  ].forEach(function(tier,i){
    var rngData=rng[tier.key]; if(!rngData)return;
    var row=el("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",
      borderBottom:i<2?"1px solid "+cl.border:"none",background:cl.surface}});
    var left=el("div",{style:{display:"flex",alignItems:"center",gap:"10px"}});
    left.appendChild(el("span",{style:{fontSize:"15px"}},tier.icon));
    var lbl=el("div",{});
    lbl.appendChild(el("div",{style:{color:tier.color,fontSize:"12px",fontWeight:"700",fontFamily:"'Space Grotesk',monospace"}},tier.label));
    lbl.appendChild(el("div",{style:{color:cl.muted,fontSize:"9px",fontFamily:"'Inter',sans-serif"}},tier.sub));
    left.appendChild(lbl); row.appendChild(left);
    var pr=el("div",{style:{textAlign:"right"}});
    pr.appendChild(el("div",{style:{color:cl.white,fontSize:"12px",fontWeight:"700",fontFamily:"'Space Grotesk',monospace"}},
      fmtP(rngData[0])+" – "+fmtP(rngData[1])));
    row.appendChild(pr); rangeCard.appendChild(row);
  });
  qc.appendChild(rangeCard);
  // Full Analyzer CTA
  var fullBtn=el("button",{style:{width:"100%",marginTop:"12px",padding:"10px",borderRadius:"10px",border:"1px solid "+cl.goldDim,background:"transparent",color:cl.gold,fontSize:"12px",fontWeight:"600",fontFamily:"'Space Grotesk',monospace",cursor:"pointer"}});
...
function _renderQuickCheckWidget(cl, opts){
  ...
  if(opts.showTitle!==false){
    qc.appendChild(el("div",{style:{textAlign:"center",marginBottom:"16px"}},[
      ...
      el("div",{style:{color:cl.sub,fontSize:"11.5px",fontFamily:"'Inter',sans-serif"}},"Area, beds & budget — get your top 3 building picks instantly")
    ]));
  }
  if(!window._qcState)window._qcState={area:"",beds:"2 BR",building:"",price:"",buildingPicks:null,rangeResult:null,mode:"sale"};
  if(!window._qcState.beds)window._qcState.beds="2 BR";
  if(!("rangeResult" in window._qcState))window._qcState.rangeResult=null;
  if(!("buildingPicks" in window._qcState))window._qcState.buildingPicks=null;
  ...
  ["sale","rent"].forEach(function(m){
    ...
    tb.textContent=m==="sale"?"BUY":"RENT";
    tb.addEventListener("click",function(){qs.mode=m;qs.buildingPicks=null;qs.rangeResult=null;render();});
  ...
  qcAreaInp.addEventListener("input",function(){
    qs.area=this.value;qs.buildingPicks=null;qs.rangeResult=null;
    ...
    hits.slice(0,8).forEach(function(n){
      ...
      row.textContent=n;
      row.addEventListener("mousedown",function(e){e.preventDefault();qs.area=n;qs.buildingPicks=null;qs.rangeResult=null;render();});
  ...
  ["Studio","1 BR","2 BR","3 BR","4 BR","5+ BR"].forEach(function(b){
    ...
    bb.textContent=b;
    bb.addEventListener("click",function(){qs.beds=b;qs.buildingPicks=null;qs.rangeResult=null;render();});
    bedsRow.appendChild(bb);
  });
  qc.appendChild(bedsRow);
  // Budget input — now the tool's central input (2026-07-14 redesign): the
  // budget is what drives the building recommendations below, not an
  // optional add-on to a generic area range.
  ...
  pLabel.textContent=qs.mode==="rent"?"YOUR ANNUAL RENT BUDGET":"YOUR BUDGET";
  ...
  pInp.addEventListener("input",function(){qs.price=this.value.replace(/[^0-9]/g,"");this.value=qs.price?parseInt(qs.price).toLocaleString():"";qs.buildingPicks=null;});
  qc.appendChild(pInp);
  // Check button
  ...
  checkBtn.textContent=qs.mode==="rent"?"FIND BUILDINGS TO RENT":"FIND BUILDINGS TO BUY";
  checkBtn.addEventListener("click",function(){
    var _eOld=document.getElementById("_qc_area_err");if(_eOld)_eOld.remove();
    var _eOld2=document.getElementById("_qc_budget_err");if(_eOld2)_eOld2.remove();
    if(!qs.area||!AREAS[qs.area]){
      var _e=el("div",{id:"_qc_area_err",style:{color:"#EF4444",fontSize:"11px",fontFamily:"'Inter',sans-serif",marginBottom:"8px",padding:"8px 12px",background:"rgba(239,68,68,0.07)",borderRadius:"8px",border:"1px solid rgba(239,68,68,0.3)"}});
      _e.textContent="Please select a valid area first.";checkBtn.insertAdjacentElement("beforebegin",_e);
      return;
    }
    var price=parseInt((qs.price||"").replace(/[^0-9]/g,""));
    var minBudget=qs.mode==="rent"?5000:50000;
    if(!price||price<minBudget){
      var _e2=el("div",{id:"_qc_budget_err",style:{color:"#EF4444",fontSize:"11px",fontFamily:"'Inter',sans-serif",marginBottom:"8px",padding:"8px 12px",background:"rgba(239,68,68,0.07)",borderRadius:"8px",border:"1px solid rgba(239,68,68,0.3)"}});
      _e2.textContent="Please enter your budget.";checkBtn.insertAdjacentElement("beforebegin",_e2);
      return;
    }
    qs.rangeResult=computeAreaPriceRange(qs.area,qs.beds||"2 BR",qs.mode);
    qs.buildingPicks=_qcRecommendBuildings(qs.area,qs.beds||"2 BR",qs.mode,price);
    render();
  });
...
// Beta launch (2026-07-15): the Analyzer page opened with two separate
// entry points at once (this AI Smart Search text/voice bar, then the
// building/cluster search box right below it) — confusing for first-time
// beta users deciding which one to use. Hidden (not deleted) per the site
// owner's explicit request so the Analyzer flow is just the single,
// familiar building/cluster search — flip back to true to restore the AI
// bar once beta users have more onboarding/context around it.
var ANALYZER_AI_SEARCH_ENABLED=false;
...
function renderAnalyzer(){
  ...
  // --- AI SMART SEARCH (compact bar) --- hidden for beta, see
  // ANALYZER_AI_SEARCH_ENABLED at the top of this file.
  if(analyzerState.stage===0&&ANALYZER_AI_SEARCH_ENABLED){
    if(!window._aiSearch)window._aiSearch={text:"",parsing:false,parsed:null,missing:[],filled:[]};
    var ai=window._aiSearch;
    var aiSysPrompt='You are DubAIVal Smart Search — a Dubai property parser that understands natural language in English, Arabic, and Farsi.\nExtract these fields and return ONLY a JSON object (no markdown, no explanation):\n{"area":null,"building":null,"propType":null,"beds":null,"size_sqft":null,"floor":null,"view":null,"furnished":null,"parking":null,"bathrooms":null,"price":null,"purpose":null}\n\nFIELD RULES:\n- propType: apartment, villa, townhouse, penthouse, office, land\n- beds: "Studio","1 BR","2 BR","3 BR","4 BR","5 BR","5+ BR"\n- furnished: Furnished/Unfurnished/Semi-Furnished\n- view: Full Sea View, Partial Sea View, Burj Khalifa View, Fountain View, Marina View, Canal View, Golf View, Garden View, Pool View, Skyline View, Community View, Palm View, Creek View, Boulevard View, Sheikh Zayed Road View, Lake View, Lagoon View\n- purpose: sale or rent\n- price: number in AED (convert M=million, K=thousand)\n- area: use official DLD area names (e.g. "Dubai Marina" not "Marina", "Downtown Dubai" not "Downtown", "Jumeirah Village Circle" not "JVC")\n- building: full official name if recognizable\n\nMULTI-LANGUAGE PARSING:\nArabic: غرفة/غرفتين/ثلاث غرف=1/2/3 BR, شقة=apartment, فيلا=villa, تاون هاوس=townhouse, بنتهاوس=penthouse, مكتب=office, أرض=land, مارينا=Dubai Marina, داون تاون=Downtown Dubai, بيزنس باي=Business Bay, جي في سي=Jumeirah Village Circle, طابق=floor, إطلالة بحرية=Full Sea View, مفروش=Furnished, للبيع=sale, للإيجار=rent\nFarsi: اتاق خواب/خوابه=bedroom, آپارتمان=apartment, ویلا=villa, متر مربع=sqm(×10.764=sqft), طبقه=floor, نمای دریا=Sea View, مبله=Furnished, فروشی=sale, اجاره=rent, مارینا=Dubai Marina, داون تاون=Downtown Dubai, پالم=Palm Jumeirah\nHindi: बेडरूम/BHK=bedroom, फ्लैट=apartment, विला=villa, मंज़िल=floor, समुद्र दृश्य=Sea View, बिक्री=sale, किराया=rent, मरीना=Dubai Marina, डाउनटाउन=Downtown Dubai\nUrdu: کمرہ=bedroom, فلیٹ=apartment, ویلا=villa, منزل=floor, سمندر نظارہ=Sea View, فروخت=sale, کرایہ=rent\nRussian: спальня/комната=bedroom, квартира=apartment, вилла=villa, этаж=floor, вид на море=Sea View, продажа=sale, аренда=rent, Марина=Dubai Marina, Даунтаун=Downtown Dubai\nChinese: 卧室/房间=bedroom, 公寓=apartment, 别墅=villa, 楼层=floor, 海景=Sea View, 出售=sale, 出租=rent, 迪拜码头=Dubai Marina\nFrench: chambre=bedroom, appartement=apartment, villa=villa, étage=floor, vue mer=Sea View, vente=sale, location=rent\nTurkish: yatak odası=bedroom, daire=apartment, villa=villa, kat=floor, deniz manzarası=Sea View, satılık=sale, kiralık=rent\nGerman: Schlafzimmer=bedroom, Wohnung=apartment, Villa=villa, Etage=floor, Meerblick=Sea View, Verkauf=sale, Miete=rent\nSpanish: dormitorio/habitación=bedroom, apartamento/piso=apartment, villa=villa, planta=floor, vista al mar=Sea View, venta=sale, alquiler=rent\nMalayalam: ബെഡ്റൂം=bedroom, ഫ്ലാറ്റ്=apartment, വില്ല=villa, നില=floor, കടൽ കാഴ്ച=Sea View, വിൽപ്പന=sale, വാടക=rent\nTamil: படுக்கையறை=bedroom, அடுக்குமாடி=apartment, வில்லா=villa, தளம்=floor, கடல் காட்சி=Sea View, விற்பனை=sale, வாடகை=rent\nPunjabi: ਕਮਰਾ/ਬੈੱਡਰੂਮ=bedroom, ਫਲੈਟ=apartment, ਵਿਲਾ=villa, ਮੰਜ਼ਿਲ=floor, ਵਿਕਰੀ=sale, ਕਿਰਾਇਆ=rent\nIndonesian: kamar tidur=bedroom, apartemen=apartment, vila=villa, lantai=floor, pemandangan laut=Sea View, dijual=sale, disewakan=rent\nDutch: slaapkamer=bedroom, appartement=apartment, villa=villa, verdieping=floor, zeezicht=Sea View, te koop=sale, te huur=rent\nPolish: sypialnia=bedroom, mieszkanie=apartment, willa=villa, piętro=floor, widok na morze=Sea View, sprzedaż=sale, wynajem=rent\nRomanian: dormitor=bedroom, apartament=apartment, vilă=villa, etaj=floor, vedere la mare=Sea View, vânzare=sale, chirie=rent\n\nIf a field is not mentioned, set it to null. Be smart about context: "2 bed marina sea view 2.5M" → area:Dubai Marina, beds:2 BR, view:Full Sea View, price:2500000, purpose:sale.';
  ...
  if(!f.building||!f.propCategory){
    ...
    if(window._qcElement){
      if(window._qcState&&window._qcState.buildingPicks)window._qcExpanded=true;
  ...
  if(f.propCategory==="villa"){
    ...
    if(true){
      submitBtn.addEventListener("click",function(){
        ...
        if(!f.size)_mis.push("Property size (sqft)");
        if(!f.price)_mis.push(isRental?"Asking annual rent":"Asking price");
        if(_mis.length){
...
function renderAnalyzerResult(wrap){
  ...
  (function(){
    ...
    liqWrap.appendChild(div({display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"4px"},[
    ...
    // Clarifies these 3 figures are AREA-wide (real Bayut/DLD-sourced), not
    // building-specific — added 2026-07-27 after a real user report that two
    // different buildings in the same area showed identical numbers here.
    // Confirmed via direct testing this is correct, not a bug: dom/txVol are
    // genuinely area-level data (matches the "AREA AVG. DAYS ON MARKET" label
    // already used in the Listing Pitch card) — a building-specific version
    // of this exact signal is the separate "Building Turnover Rate" card
    // directly below, which already uses real per-building unit/trade counts.
    liqWrap.appendChild(div({color:cl.sub,fontSize:"9.5px",fontFamily:"'Inter',sans-serif",marginBottom:"10px"},"Area-wide figures for "+(f.area||"this area")+" — see Building Turnover Rate below for this specific building"));
    // Liquidity score bar
    var barOuter=el("div",{style:{background:cl.border,borderRadius:"6px",height:"8px",marginBottom:"12px",overflow:"hidden"}});
    ...
    [{l:"Avg. Days on Market (Area)",v:val.domEst+" days",c:val.domEst<=30?cl.green:val.domEst<=60?cl.yellow:cl.red},
  ...
  (function(){
    ...
    var gaugeOuter=el("div",{style:{background:cl.border,borderRadius:"6px",height:"8px",marginBottom:"12px",overflow:"hidden"}});
  ...
  (function(){
    ...
    [{l:"Price Gap",v:val.priceGapScore,w:"50%",icon:"",d:pgDesc},
     ...
     {l:"Market Depth",v:val.marketDepthScore,w:"30%",icon:"",d:mdDesc}
      ...
      // Mini bar
      var miniBar=el("div",{style:{background:cl.border,borderRadius:"3px",height:"3px",marginTop:"6px",overflow:"hidden"}});
  ...
  if(val.geoScore){(function(){
    ...
    var gaugeOuter=el("div",{style:{background:cl.border,borderRadius:"8px",height:"10px",marginBottom:"16px",overflow:"hidden",position:"relative"}});
    ...
    function renderAmenities(data,fetchFailMsg){
      ...
      Object.keys(cfg).forEach(function(k){
        ...
        if(a){
          ...
          var bar=el("div",{style:{background:cl.border,borderRadius:"3px",height:"3px",marginTop:"6px",overflow:"hidden"}});
          bar.appendChild(el("div",{style:{width:pct+"%",height:"100%",background:a.dist<=500?c:cl.border,borderRadius:"3px"}}));
          box.appendChild(bar);
        }else{
  ...
  (function(){
    ...
    if(already||WS.done){
      ...
      const inp=el("input",{type:"email",placeholder:"your@email.com",style:{flex:"1",background:cl.raised,border:"1px solid "+cl.border,color:cl.white,padding:"10px 12px",borderRadius:"8px",fontSize:"13px",fontFamily:"'Inter',sans-serif",outline:"none",boxSizing:"border-box"}});
  ...
  */
  ...
  (function(){
    ...
    function metCard(label,value,color){
      var c=el('div',{style:{background:cl.surface,border:'1px solid '+cl.border,borderRadius:'10px',padding:'10px'}});
    ...
    scenLabel.appendChild(span({color:cl.gold,fontSize:'11px',letterSpacing:'0.12em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace"},'Scenario Comparison'));
    ...
    chartLabel.appendChild(span({color:cl.gold,fontSize:'11px',letterSpacing:'0.12em',textTransform:'uppercase',fontFamily:"'Space Grotesk',monospace"},'Equity Growth'));
  ...
  var liText="Just valued a property in "+f.area+" using DubAIVal AI — "+val.verdict+" at AED "+val.fairPrice.toLocaleString()+". The platform tracks 11,700+ properties across 347 areas. #DubaiRealEstate #PropTech";
  ...
  [{k:'name',ph:'Your Name (optional)'},{k:'company',ph:'Company (optional)'},{k:'rera',ph:'RERA No. (optional)'}].forEach(function(fld){
    const inp=el('input',{type:'text',placeholder:fld.ph,style:{background:cl.raised,border:'1px solid '+cl.border,color:cl.white,padding:'8px 10px',borderRadius:'8px',fontSize:'11px',fontFamily:"'Inter',sans-serif",width:'100%',outline:'none',boxSizing:'border-box'}});
  ...
  if(!window.PDF_UNLOCKED){
    ...
    const eInp=el('input',{type:'email',placeholder:'Enter email to unlock free PDF',style:{flex:'1',background:cl.raised,border:'1px solid '+cl.border,color:cl.white,padding:'10px 12px',borderRadius:'8px',fontSize:'13px',fontFamily:"'Inter',sans-serif",outline:'none'}});
