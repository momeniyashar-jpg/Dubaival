...
// Populates DV_AUTH.profile.is_pro/.video_credits/.video_gen_credits/
// .whatsapp_credits from user_profiles (see
// supabase-subscriptions-schema.sql, supabase-video-credits-schema.sql,
// supabase-video-gen-credits-schema.sql, supabase-whatsapp-credits-schema.sql)
// so isProUser() reflects the real Stripe subscription state instead of
// always defaulting to false — video_credits (Whisper subtitles),
// video_gen_credits (Kling/Runway/HeyGen/D-ID video generation), and
// whatsapp_credits (WhatsApp Business API send/auto-reply) are three
// separate pay-per-use balances, all added here rather than a second fetch
// since this call already runs on every sign-in. Needs the user's own
// access token (not just the anon key) since user_profiles RLS is
// row-owner-scoped, same as every other authenticated read in this file.
// Fire-and-forget — a slow/failed fetch just means isProUser() stays false
