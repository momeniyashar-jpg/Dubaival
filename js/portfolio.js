  ...
  var cmpBtn=el("button",{style:{width:"100%",background:"rgba(212,175,55,0.15)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",color:cl.gold,border:"1px solid rgba(212,175,55,0.3)",padding:"13px",borderRadius:"10px",fontSize:"13px",fontWeight:"700",fontFamily:"'Space Grotesk',monospace",cursor:"pointer"},onclick:async function(){
    ...
    try{
      ...
      var lines=filled.map(function(it,i){
        ...
        if(!d){noDataCount++;return(i+1)+". "+it.value+" — NO VERIFIED DATA (not found in our 9,309-building / 347-area database)";}
...
function renderPersonal(){
  ...
  if(p.step===0){
    ...
    [["✓","9,309 buildings & 347 DLD-verified areas"],["✓","3-year scenario: conservative · base · optimistic"],["✓","Golden Visa eligibility check (≥ AED 2M)"],["✓","Investor DNA profile + one concrete next step"]].forEach(function(b){
...
function generatePortfolioPDF(metrics,totalValue,totalPurchase,totalROI,avgGrossYield,avgNetYield,health){
  ...
  h+='</div>';
  h+='<div style="font-size:8px;color:#aaa;margin-top:16px;border-top:1px solid #eee;padding-top:8px">Valuations use DubAIVal\'s Cascade AVM with hedonic pricing (11,600+ properties &bull; 347 areas). This is an AI-generated estimate for informational purposes only and is not a RERA- or RICS-certified valuation. Not financial advice — consult a licensed advisor for investment decisions. &bull; Generated '+dateStr+'</div>';
  h+='</div>';
...
function renderPortfolio(mode){
  ...
  disc.innerHTML="<strong style='color:"+cl.gold+"'>Note:</strong> Valuations use DubAIVal's Cascade AVM with hedonic pricing (11,600+ properties · 347 areas). Portfolio data is stored locally on your device. Not financial advice — consult a licensed advisor for investment decisions.";
