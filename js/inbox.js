...
function renderInbox() {
  ...
  var titleEl = div("font-size:22px;font-weight:700;color:#fff;font-family:'Space Grotesk',sans-serif", "Inbox");
  titleRow.appendChild(titleEl);
  if (unread > 0) {
    var badge = span("background:#EF4444;color:#fff;font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px", unread + " unread");
  ...
  tabs.forEach(function(t) {
    ...
    var tabEl = div("display:flex;align-items:center;gap:6px;padding:6px 16px;border-radius:20px;font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;" + (isActive ? "background:#D4AF37;color:#000" : "background:#1A1F2E;color:#8899AA;border:1px solid #2A3040"), t.label);
    if (count > 0) {
      var c = span("background:" + (isActive ? "rgba(0,0,0,.25)" : "#2A3040") + ";border-radius:20px;padding:1px 6px;font-size:11px", String(count));
      tabEl.appendChild(c);
    }
...
function _renderItem(item) {
  ...
  var pIcon = _platformIcon(platform);
  ...
  var senderEl = span("font-weight:600;color:#fff;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px", isEmail ? (d.from_name || d.from_email || "Unknown") : (d.sender_name || d.sender_id || "Unknown"));
  topRow.appendChild(senderEl);
  if (isEmail && d.from_email) {
    var emailSpan = span("color:#556677;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis", "<" + d.from_email + ">");
    topRow.appendChild(emailSpan);
  } else if (!isEmail) {
    var typeSpan = span("font-size:11px;color:" + pColor + ";background:" + pColor + "22;padding:1px 6px;border-radius:10px;font-weight:600", d.event_type === "dm" ? "DM" : "Comment");
  ...
  if (isExpanded) {
    ...
    if (isEmail) {
      if (d.subject && d.subject !== "(No subject)") {
        body.appendChild(div("font-size:13px;font-weight:600;color:#D4AF37;margin-bottom:12px", "Subject: " + d.subject));
      }
      if (d.body_text) {
    ...
    if (isEmail && d.status !== "agent_replied") {
      ...
      replySection.appendChild(div("font-size:13px;font-weight:600;color:#fff;margin-bottom:8px", "Reply"));
      ...
      var sendBtn = div("display:flex;align-items:center;gap:6px;background:#D4AF37;color:#000;font-weight:700;font-size:13px;padding:8px 20px;border-radius:8px;cursor:pointer", INBOX_STATE.replyBusy[id] ? "Sending..." : "Send Reply");
      ...
      if (INBOX_STATE.replySent[id]) {
        btnRow.appendChild(span("color:#10B981;font-size:13px;font-weight:600", "Sent!"));
      }
      if (INBOX_STATE.replyError[id]) {
        btnRow.appendChild(span("color:#EF4444;font-size:12px", "Error: " + INBOX_STATE.replyError[id]));
      }
