...
function renderProfilePanel(){
  ...
  // ── REPLY AUTOMATION ───────────────────────────────────────────
  // Per-channel manual/automatic toggle (added 2026-07-19) — closes a real
  // gap: these 4 channels previously had NO toggle at all (the only existing
  // automation toggle, CHIEFS_AUTOMATION, governs a different pipeline —
  // the Co-pilot's match-drafting flow, not this raw auto-reply system).
  // Persisted server-side (social_credentials.auto_reply_*) since it's
  // checked inside serverless webhook handlers with no access to
  // localStorage. Every reply is generated by the SAME RAG-grounded
  // real-estate-specialist AI used across the rest of the app (api/_lib/
  // rag.js), never a generic "answers just to answer" chatbot.
  maxW.appendChild(secLabel("Reply Automation"));
  var autoCard=el("div",{style:{background:cl.raised,border:"1px solid "+cl.border,borderRadius:"12px",padding:"2px 14px",marginBottom:"20px"}});
  autoCard.appendChild(div({color:cl.sub,fontSize:"10.5px",padding:"10px 0 2px",fontFamily:"'Inter',sans-serif",lineHeight:"1.4"},"When ON, your AI Chief of Staff replies automatically — grounded in live Dubai market knowledge (real news, area data, forecasts), not a generic chatbot. Turn a channel OFF to answer it yourself from the Inbox instead."));
  function _replyToggleOn(key){return localStorage.getItem(key)!=="0";}
  function _setReplyToggle(key,v){
    if(v)localStorage.removeItem(key);else localStorage.setItem(key,"0");
    if(typeof _syncCredsToServer==="function")_syncCredsToServer();
    render();
  }
  if(typeof _chToggleRow==="function"){
    autoCard.appendChild(_chToggleRow("Email",
      "AI drafts and sends a reply to inbound emails automatically.",
      _replyToggleOn("dv_auto_reply_email"),function(v){_setReplyToggle("dv_auto_reply_email",v);}));
    autoCard.appendChild(_chToggleRow("WhatsApp",
      "AI drafts and sends a reply to inbound WhatsApp messages automatically (uses your pay-per-window credits).",
      _replyToggleOn("dv_auto_reply_whatsapp"),function(v){_setReplyToggle("dv_auto_reply_whatsapp",v);}));
    autoCard.appendChild(_chToggleRow("Instagram (DMs + comments)",
      "AI drafts and sends a reply to inbound Instagram messages and comments automatically.",
      _replyToggleOn("dv_auto_reply_instagram"),function(v){_setReplyToggle("dv_auto_reply_instagram",v);}));
    autoCard.appendChild(_chToggleRow("Facebook (messages + comments)",
      "AI drafts and sends a reply to inbound Facebook messages and comments automatically.",
      _replyToggleOn("dv_auto_reply_facebook"),function(v){_setReplyToggle("dv_auto_reply_facebook",v);},true));
  }
  maxW.appendChild(autoCard);
